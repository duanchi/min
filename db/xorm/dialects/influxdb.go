...
var (
	influxdbReservedWords = map[string]bool{
		"ADD":               true,
		"ALL":               true,
		"ALTER":             true,
		"ANALYZE":           true,
...
func (db *influxdb) Init(uri *URI) error {
	db.quoter = influxQuoter
	return db.Base.Init(db, uri)
}

func (db *influxdb) SetParams(params map[string]string) {
...
func (db *influxdb) SQLType(c *schemas.Column) string {
	var res string
...
func (db *influxdb) IsReserved(name string) bool {
	_, ok := mysqlReservedWords[strings.ToUpper(name)]
	return ok
}

func (db *influxdb) AutoIncrStr() string {
	return "AUTO_INCREMENT"
}

func (db *influxdb) IndexCheckSQL(tableName, idxName string) (string, []interface{}) {
...
func (db *influxdb) IsTableExist(queryer core.Queryer, ctx context.Context, tableName string) (bool, error) {
	sql := "SELECT `TABLE_NAME` from `INFORMATION_SCHEMA`.`TABLES` WHERE `TABLE_SCHEMA`=? and `TABLE_NAME`=?"
	return db.HasRecords(queryer, ctx, sql, db.uri.DBName, tableName)
}

func (db *influxdb) AddColumnSQL(tableName string, col *schemas.Column) string {
...
func (db *influxdb) GetColumns(queryer core.Queryer, ctx context.Context, tableName string) ([]string, map[string]*schemas.Column, error) {
...
func (db *influxdb) GetTables(queryer core.Queryer, ctx context.Context) ([]*schemas.Table, error) {
...
func (db *influxdb) SetQuotePolicy(quotePolicy QuotePolicy) {
	switch quotePolicy {
...
func (db *influxdb) GetIndexes(queryer core.Queryer, ctx context.Context, tableName string) (map[string]*schemas.Index, error) {
	args := []interface{}{db.uri.DBName, tableName}
...
func (db *influxdb) CreateTableSQL(table *schemas.Table, tableName string) ([]string, bool) {
...
func (db *influxdb) Filters() []Filter {
	return []Filter{}
}
...
func (p *influxdbDriver) Parse(driverName, dataSourceName string) (*URI, error) {
